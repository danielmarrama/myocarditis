...
def make_random_sets():
  ...
  for i in range(1000):
    with open(f'random_sets/{i+1}/random_protein_set_{i+1}.fasta', 'w') as f:
...
def search_random_sets():
  ...
  for i in range(1000):
    random_set_path = f'random_sets/{i+1}'
    ...
    Matcher('spike_9mers.fasta', f'random_protein_set_{i+1}', 5, 2, prepreprocessed_files_path=random_set_path).match()
    Matcher('shuffled_9mers.fasta', f'random_protein_set_{i+1}', 5, 2, prepreprocessed_files_path=random_set_path).match()
    Matcher('spike_15mers.fasta', f'random_protein_set_{i+1}', 7, 2, prepreprocessed_files_path=random_set_path).match()
    Matcher('shuffled_15mers.fasta', f'random_protein_set_{i+1}', 7, 2, prepreprocessed_files_path=random_set_path).match()
